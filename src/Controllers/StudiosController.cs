...
namespace PlayaApiV2.Controllers
{
    ...
    {
        private readonly StudiosRepository _repository;
        private readonly IStashAppService _stashAppService;

        public StudiosController(StudiosRepository studiosRepository, IStashAppService stashAppService)
        {
            _repository = studiosRepository;
            _stashAppService = stashAppService;
        }

        [HttpGet("studios")]
        [TypeFilter(typeof(AuthenticationFilter))]
        public async Task<Rsp<Page<StudioListView>>> GetStudios(
            ...
            )
        {
            ...
            if (!isAuthenticated)
            {
                return new Rsp<Page<StudioListView>>(ApiStatus.From(ApiStatusCodes.OK), new Page<StudioListView>
                {
                    Content = new List<StudioListView>(),
                    PageIndex = pageIndex,
                    PageSize = pageSize,
                    ItemTotal = 0,
                    PageTotal = 0
            ...
            var result = await _repository.GetStudiosAsync(query);
            return result;
        }

        [HttpGet("studio/{id}")]
        [TypeFilter(typeof(AuthenticationFilter))]
        public async Task<Rsp<StudioView>> GetStudio(string id)
        {
            var result = await _repository.GetStudioAsync(id);
            return result;
        }

        [HttpGet("studio/{id}/image")]
        public async Task<IActionResult> GetStudioImage(string id)
        {
            try
            {
                ...
                // Get the studio directly from StashApp to get the original image URL
                var studio = await _stashAppService.GetStudioAsync(id);
                if (studio?.Preview == null)
                {
                    return NotFound();
                }

                // Construct the original StashApp URL
                ...
                var stashAppImageUrl = $"{config.Value.Url}/studio/{id}/image";
